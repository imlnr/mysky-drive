export const CREATE_FOLDER_REQUEST = "CREATE_FOLDER_REQUEST";
export const CREATE_FOLDER_SUCCESS = "CREATE_FOLDER_SUCCESS";
export const CREATE_FOLDER_FAILURE = "CREATE_FOLDER_FAILURE";

export const GET_FOLDERS_SUCCESS = "GET_FOLDERS_SUCCESS";
export const UPDATE_FOLDER_SUCCESS = "UPDATE_FOLDER_SUCCESS";
export const SOFT_DELETE_FOLDER_SUCCESS = "DELETE_FOLDER_SUCCESS";

export const GET_USER_REQUEST = "GET_USER_REQUEST";
export const GET_USER_SUCCESS = "GET_USER_SUCCESS";
export const GET_USER_FAILURE = "GET_USER_FAILURE";

export const GET_USER_LOGOUT = "GET_USER_LOGOUT";

export const UPLOAD_FILES_REQUEST = "UPLOAD_FILES_REQUEST";
export const UPLOAD_FILES_SUCCESS = "UPLOAD_FILES_SUCCESS";
export const UPLOAD_FILES_FAILURE = "UPLOAD_FILES_FAILURE";

export const GET_FILES_SUCCESS = "GET_FILES_SUCCESS";
export const SOFT_DELETE_FILE_SUCCESS = "DELETE_FILE_SUCCESS";
export const UPDATE_FILE_SUCCESS = "UPDATE_FILE_SUCCESS";

export const GET_TRASHED_ITEMS_REQUEST = "GET_TRASHED_ITEMS_REQUEST";
export const GET_TRASHED_ITEMS_SUCCESS = "GET_TRASHED_ITEMS_SUCCESS";
export const GET_TRASHED_ITEMS_FAILURE = "GET_TRASHED_ITEMS_FAILURE";

export const RESTORE_FOLDER_SUCCESS = "RESTORE_FOLDER_SUCCESS";